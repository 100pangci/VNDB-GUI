...
- **🛡️ 自动过滤** — Windows 非法字符自动替换为全角等效字符（`:?/\*"<>|`）
...
3. **🇨🇳 选择中文发行版** — 在右侧「中文发行」列表点击对应的汉化版本（汉化组、补丁日期、语言自动填入）
4. **📋 预览与复制** — 确认文件名预览无误后点击「一键复制」，也可点击「复制页面链接」快速获取当前 VN 的 VNDB 页面地址
